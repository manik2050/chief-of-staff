...
A personal chief-of-staff OS for [Claude Code](https://docs.claude.com/en/docs/claude-code)
(and Cursor). Clone it, run `./install.sh`, open your agent, and type `/gm`.
...
| `commands/` | Executable playbooks: `/gm`, `/triage`, `/my-tasks`, `/enrich`. |
| `core/paths.py` | One path contract for everything, plus a generated `paths.json`. |
| `install.sh` | Idempotent, non-destructive install into `~/.claude/`. |

## The four commands

**`/gm`** — the morning briefing. One screen: today's agenda, Tier 1 items with a recommended
action each, what is waiting on whom, which relationships are going cold, where the calendar
collides with your protected time. Ends with one question: what do you want to take first.
...
├── commands/
│   ├── gm.md  triage.md  my-tasks.md  enrich.md
...
1. **Rewrite `goals.yaml`.** This is the whole system. The shipped goals belong to a fictional
   founder mid-raise; they make the format obvious and are useless to you. Five to seven goals,
   each with a `why` that names the consequence of failure — triage quality comes directly from
   that field.
...
The command playbooks are plain markdown and work anywhere an agent can read files. Point your
Cursor rules at the installed OS, or symlink it:

```bash
ln -s ~/.claude/chief-of-staff/CLAUDE.md ./AGENT-OS.md
```

MCP servers are configured per client; `docs/mcp-servers.md` covers both.
